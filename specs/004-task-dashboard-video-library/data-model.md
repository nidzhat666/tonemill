...
| `result_key` | string \| null | Current object key of the graded result, `results/{folder-slug\|"unsorted"}/{display_name}` (FR-019, research.md #5). Updated in place whenever the video is moved between folders. Null until `status = done`. |
...
- `display_name` and `result_key` are set exactly once, when the worker's grading pass finishes successfully — never recomputed afterward except for the `results/...` prefix portion of `result_key` when `folder_id` changes (research.md #5).
...
**State transitions**: none beyond create/delete. Deleting a folder (FR-015) is a two-step operation: every `Video` document with that `folder_id` is updated to `folder_id = null` (and its `result_key` moved back under `results/unsorted/...`, research.md #5), then the `Folder` document itself is removed.
