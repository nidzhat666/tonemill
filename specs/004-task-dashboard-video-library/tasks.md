...
**Post-implementation amendment (2026-08-21)** — T017, T025, T026 as checked off below implemented storage-mirroring (`results/{folder-slug|"unsorted"}/{display_name}`, `copy_object`+`delete_object` on every move) exactly as originally specified. Production measured folder moves at ~2s each (S3 copy+delete on the network path). Per spec.md's Session 2026-08-21 clarification, this was reverted: results now upload to a permanent, opaque `results/{job_id}/{uuid}.mp4` (T017), and `videos/relocate.py` (T025/T026) is a `Video.folder_id`-only Mongo write with no S3 calls. The readable download name is now delivered via `S3StorageClient.presign_get_object`'s new `filename` param (`Content-Disposition` override) in both `GET /jobs/{id}` and `GET /videos` — see research.md #5 for the full rationale. No task IDs or checkboxes above were renumbered; this note is the record of the revision.
