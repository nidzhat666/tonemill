...
async def test_max_quality_swaps_cq_value_on_gpu_profile():
    # Given the GPU profile
    profile = HlgGpuProfile(Settings())

    # When building the command with max_quality off vs. on
    default_cmd = await profile.build_command(Path("in.mov"), Path("out.mp4"), max_quality=False)
    near_lossless_cmd = await profile.build_command(
        Path("in.mov"), Path("out.mp4"), max_quality=True
    )
...
async def test_max_quality_is_a_single_pass_not_a_second_encode():
    # Given the GPU profile with max_quality on
    profile = HlgGpuProfile(Settings())

    # When building the command
    cmd = await profile.build_command(Path("in.mov"), Path("out.mp4"), max_quality=True)

    # Then it's one ffmpeg invocation, not a second pass over already-encoded output
    assert cmd.count("-i") == 1


async def test_cpu_profile_ignores_max_quality_without_error():
